...
# Enable observations of different formats (including string for text)
...
class DummyVecEnv(VecEnv):
    ...
    def __init__(self, env_fns: List[Callable[[], gym.Env]]):
        ...
        self.buf_obs = OrderedDict(
            [(k, [[] for i in range(self.num_envs)]) for k in self.keys]
        )
    ...
    def step_wait(self) -> VecEnvStepReturn:
        for env_idx in range(self.num_envs):
            (
                ...
                self.buf_truncated[env_idx],
                self.buf_infos[env_idx],
            ) = self.envs[env_idx].step(self.actions[env_idx])
            if self.buf_dones[env_idx] or self.buf_truncated[env_idx]:
                # save final observation where user can get it, then reset
                self.buf_infos[env_idx]["terminal_observation"] = obs
                try:  # for evaluation only
                    obs, infos = self.envs[env_idx].reset()
                except IndexError:
                    continue
